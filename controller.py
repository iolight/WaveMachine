...
MUX_BOARD_PINS = [10, 22, 27, 17]
MUX_CHANNEL_PINS = [19, 13, 6, 5]
...
class Controller:
    ...
    def __init__(self):
        self.c_data = CData(
            ...
            theoretical_position=[0] * 100,
            real_position=[0] * 100,
            calibration=[None] * 100)
        ...
        for pin in MUX_BOARD_PINS + MUX_CHANNEL_PINS:
            GPIO.setup(pin, GPIO.OUT)           # all mux pins
    ...
    # gets data to convert on_time to mm/sec
    def calibrate_servo(self, pinx: int, piny: int):
    ...
    def _set_pin_pwm(self, pin_num: int, on_time_us: int) -> float:
        ...
        self.c_data.real_position[pin_num] += self.c_data.last_real_on_time[pin_num] * (
            sync_time - self.c_data.last_time[pin_num])
        if abs(on_time_us) < MIN_ON_TIME and on_time_us != 0:
            if (self.c_data.real_position[pin_num] < self.c_data.theoretical_position[pin_num])\
                    ^ (on_time_us < 0):
    ...
    def _set_mux(pinx: int, piny: int):
        ...
        pin_num = Controller._get_pin_num(pinx, piny)
        board_num, channel_num = Controller._get_board_and_channel(pin_num)
        for i, bit in enumerate(bin(board_num)[2:]):
            GPIO.output(MUX_BOARD_PINS[i], bool(bit))
        for i, bit in enumerate(bin(channel_num)[2:]):
            GPIO.output(MUX_CHANNEL_PINS[i], bool(bit))
